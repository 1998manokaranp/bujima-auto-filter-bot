...
async def start(client, message):
    ...
    if not await is_subscribed(client, user_id) and not vj:
        ...
        try:
            # Check if user gave an extra argument (/start something)
            if len(message.command) > 1 and message.command[1] != "subscribe":
                ...
                if REQUEST_TO_JOIN_MODE:
                    if TRY_AGAIN_BTN:
                        try:
                            ...
                            buttons.append([
                                InlineKeyboardButton("↻ ᴛʀʏ ᴀɢᴀɪɴ", url=f"https://t.me/{temp.U_NAME}?start={arg}")
                            ])
                else:
                    try:
                        kk, file_id = arg.split("_", 1)
                ...
                buttons.append([
                    InlineKeyboardButton("↻ ᴛʀʏ ᴀɢᴀɪɴ", url=f"https://t.me/{temp.U_NAME}?start=subscribe")
                ])
...
# By ThiruXD
@Client.on_message(filters.command("fsub_channels") & filters.user(ADMINS))
async def show_chandnels(client, message):
    channels = await get_force_sub_channels()
    buttons = [
        [InlineKeyboardButton((await client.get_chat(c['chat_id'])).title, callback_data=f"view_channel:{c['chat_id']}")]
        for c in channels
    ]
